...
- 应用树：应用树是各个应用之间的继承关系图，类似Java类的继承关系。应用可以多层继承，比如：customer（会员系统）--> core-domain（核心领域）--> common（公共配置）。应用树可以使不同应用之间共享配置key。

- 环境树：环境树是各个环境之间的继承关系图，类似Java类的继承关系。环境可以多层继承，比如：futureTest1（特性测试环境1）--> test（测试环境）--> offline（线下环境）。环境树可以使一个应用在不同环境之间共享配置value。

##### 每个配置项可以有作用域：私有、可继承、公开

- 私有：私有的配置只能被当前应用读取，其他应用无法读取，类似Java类的private访问级别字段。这种作用域可以保护配置的安全性。
...
> 访问权限只约束普通管理员，不会约束超级管理员。运维人员使用超级管理员账号把敏感配置的权限设置为“只读”或“无”，然后给开发人员创建普通管理员账号，让开发人员也能查看和修改非敏感配置（比如一些业务开关配置），而不需要再通过运维人员这一层，进而提高开发人员和运维人员生产力。
