...
启动服务端命令模板：
```shell
nohup java -jar configcenter-1.5.2.RELEASE.jar --spring.profiles.active="online" --spring.datasource.url="数据库url" --spring.datasource.username="数据库用户名" --spring.datasource.password="数据库密码" --spring.redis.host="redis的地址" --spring.redis.port="redis的端口" &
```
比如我本地测试时启动命令：
```shell
nohup java -jar configcenter-1.5.2.RELEASE.jar --spring.profiles.active="online" --spring.datasource.url="jdbc:mysql://localhost:3306/configcenter-dev?useUnicode=true&characterEncoding=utf-8" --spring.datasource.username="root" --spring.datasource.password="root" --spring.redis.host="localhost" --spring.redis.port="6379" &
```
